...
vec4 pointLight() {
    ...
    return (texture(Texture0, v_texCoord) * (diffuse * intensity + ambient) + texture(Texture1, v_texCoord).r * specular * intensity) * LightColour;
}
...
vec4 directLight() {
    ...
    return (texture(Texture0, v_texCoord) * (diffuse + ambient) + texture(Texture1, v_texCoord).r * specular) * LightColour;
}
